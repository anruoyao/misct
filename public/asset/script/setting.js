$(document).ready(function () {
  ...
  $("#maintenance_mode_enabled").on("change", function () {
    let isMaintenanceModeEnabled = $(this).is(":checked") ? 1 : 0;
    $(this).val(isMaintenanceModeEnabled);
    $("#is_maintenance_mode").val(isMaintenanceModeEnabled);
  });
  $("#force_app_update_enabled").on("change", function () {
    let isForceAppUpdateEnabled = $(this).is(":checked") ? 1 : 0;
    $(this).val(isForceAppUpdateEnabled);
    $("#is_force_app_update").val(isForceAppUpdateEnabled);
  });

  $(document).on("submit", "#maintenanceForm", function (e) {
    e.preventDefault();
    if (user_type == 1) {
      let formData = new FormData($("#maintenanceForm")[0]);
      $.ajax({
        type: "POST",
        url: `${domainUrl}updateSettings`,
        data: formData,
    ...
    $(document).on("submit", "#forceAppUpdateForm", function (e) {
    e.preventDefault();
    if (user_type == 1) {
      let formData = new FormData($("#forceAppUpdateForm")[0]);
      $.ajax({
        type: "POST",
        url: `${domainUrl}updateSettings`,
        data: formData,
