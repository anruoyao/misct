...
/*
|--------------------------------------------------------------------------
| Email Authentication (本地化邮箱登录，替代 Firebase)
|--------------------------------------------------------------------------
| 注册 / 登录 / 重发验证 / 找回密码 走 checkHeader 中间件，由 App 调用。
| 邮箱验证链接和密码重置链接由用户从邮件点击进入浏览器，不带 apikey，
| 因此不走 checkHeader。
*/
Route::post('emailRegister', [EmailAuthController::class, 'register'])->middleware('checkHeader');
Route::post('emailLogin', [EmailAuthController::class, 'login'])->middleware('checkHeader');
Route::post('resendVerification', [EmailAuthController::class, 'resendVerification'])->middleware('checkHeader');
Route::post('forgotPassword', [EmailAuthController::class, 'forgotPassword'])->middleware('checkHeader');

// 邮件链接落地：GET，由浏览器访问，不校验 apikey
Route::get('verifyEmail/{id}/{hash}', [EmailAuthController::class, 'verify'])
    ->name('api.verifyEmail');
Route::get('resetPassword/{token}', [EmailAuthController::class, 'showResetForm'])
    ->name('api.resetPassword.show');
Route::post('resetPassword', [EmailAuthController::class, 'resetPassword'])
    ->name('api.resetPassword.submit');
